...
//
// Program: Project 9 - sort_boxes_with_contents.c
//
// Description: Sorts a database of boxes input by
//				a text file of dimensions and contents.
//
...
int main(void)
{
    ...
    if((instream = fopen("boxes_with_contents.txt", "r")) == '\0')                                  //Opens the input file, while doing some error-checking as well
    {
        ...
        &myBoxes[i].width, &myBoxes[i].height, &myBoxes[i].weight) != EOF; i++)
    {
        ...
        fgets(myBoxes[i].contents, MAX, instream);                                                  //Retrieves the string containing the contents of the box
    ...
    if((outstream = fopen("sorted_boxes_with_contents.txt", "w")) == '\0')                          //Opens the output file, while doing some error-checking as well
    {
        printf("\nThere was an error opening the new file.\n\n");                                   //If there is an error when opening the file, print a message
        return 1;                                                                                   //And return 1
    }

    fputs("#\tLength\t\tWidth\t\tHeight\t\tVolume\t\tWeight\t\tContents\n\n", outstream);           //Places the column headers in the newly created file

    for(i = 0; i < count; i++)                                                                      //'for' loop to print the data from each structure in the array
    {
        if(fprintf(outstream, "%d\t%.6lf\t%.6lf\t%.6lf\t%.6lf\t%.6lf\t%s\n", i, myBoxes[i].length, 
            myBoxes[i].width, myBoxes[i].height, myBoxes[i].volume, myBoxes[i].weight, 
            myBoxes[i].contents) == EOF)
        {
    ...
    printf("\n'sorted_boxes_with_contents.txt' was saved successfully.\n\n");                       //Informs the user of the program's accomplishment
